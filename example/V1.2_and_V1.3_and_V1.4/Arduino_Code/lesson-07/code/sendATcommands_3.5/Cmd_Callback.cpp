 ...
 * The command handlers set these flags so the foreground loop can
 * finish the actual radio work outside the parser.
 *--------------------------------------------------------------*/
...
/**
 * @brief Convert one ASCII hex digit to a numeric value.
 *
 * @param ascii Input character.
...
/**
 * @brief Print a 32-bit value as colon-separated hex bytes.
 *
...
/**
 * @brief Print a 64-bit value as colon-separated hex bytes.
 *
 * @param value Value to print.
 * @return None.
 */
...
/**
 * @brief Print a 16-byte array as colon-separated hex bytes.
 *
 * @param pt Byte array to print.
 * @return None.
 */
static void print_16_02x(uint8_t *pt);

/**
 * @brief Print an 8-byte array as colon-separated hex bytes.
 *
 * @param pt Byte array to print.
 * @return None.
 */
...
/**
 * @brief Parse a colon-separated hex string into raw bytes.
 *
 * @param param Textual parameter list.
 * @param Buff Output buffer for binary bytes.
 * @param lenth Number of bytes to decode.
...
bool hexStringToUint8Array(const char *hexStr, uint8_t *MsgTemp, size_t *actualSize) {
    if (hexStr == NULL || actualSize == NULL) {
        return false;
    }
    ...
    *actualSize = 0;
    ...
    if (length == 0 || (length % 2) != 0 || (length / 2) > sizeof(::MsgTemp)) {
        return false;
    }

    for (size_t i = 0; i < length / 2; i++) {
        int8_t high = ascii_4bit_to_hex((uint8_t)hexStr[i * 2]);
        int8_t low = ascii_4bit_to_hex((uint8_t)hexStr[i * 2 + 1]);
        if (high < 0 || low < 0) {
            *actualSize = 0;
            return false;
        }
        MsgTemp[i] = (uint8_t)((high << 4) | low);
        (*actualSize)++;
    }
    return true;
}
...
ATEerror_t AT_Send_set(const char *param) {
  ...
  while (paramLength > 1 && (paramCopy[paramLength - 2] == '\r' || paramCopy[paramLength - 2] == '\n')) {
    paramCopy[--paramLength - 1] = '\0';
  }
  ...
  if (Port_Number == 0 || Port_Number > 223) {
    return AT_PARAM_ERROR;
  }
  ...
  char *numberEnd = NULL;
  long secondField = strtol(token, &numberEnd, 10);
  if (numberEnd == token || *numberEnd != '\0' || secondField < 0 || secondField > 223) {
    return AT_PARAM_ERROR;
  }
  bool lengthMode = secondField > 1;
  Ack_State = lengthMode ? 0 : (uint8_t)secondField;
  Serial.print(lengthMode ? "Declared_Length: " : "Ack_State: ");
  Serial.println(secondField);
  token = strtok(NULL, ",:");
  if (token == NULL || !hexStringToUint8Array(token, MsgTemp, &MsgTemp_size)) {
    return AT_PARAM_ERROR;
  }
  if (lengthMode && secondField != (long)strlen(token) && secondField != (long)MsgTemp_size) {
    return AT_PARAM_ERROR;
  }
